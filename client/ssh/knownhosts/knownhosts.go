...
// 
// Downloaded original file from:
// https://github.com/golang/crypto/blob/master/ssh/knownhosts/knownhosts.go
//
// Modified by Milan Pandurov
// Replaced net.SplitHostPort with function that works with SCION addresses
//
package knownhosts
...
func (db *hostKeyDB) IsHostAuthority(remote ssh.PublicKey, address string) bool {
    h, p, err := scionutils.SplitHostPort(address)
    if err != nil {
        return false
    }
...
func newHostnameMatcher(pattern string) (matcher, error) {
    ...
    for _, p := range strings.Split(pattern, "#") { //Fixme(milan): using random character
        ...
        var err error
        a.host, a.port, err = scionutils.SplitHostPort(p)
        if err != nil {
            a.host = p
            a.port = "22"
        }
...
func (db *hostKeyDB) check(address string, remote net.Addr, remoteKey ssh.PublicKey) error {
    ...
    host, port, err := scionutils.SplitHostPort(remote.String())
    if err != nil {
    ...
    if address != "" {
        host, port, err := scionutils.SplitHostPort(address)
        if err != nil {
